...
export default function ProjectsList() {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (response.ok) {
        Alert.alert('Success', 'projects deleted successfully', [{ text: 'OK', onPress: () => setRefreshPage(!refreshPage) }]);
      } else {
  ...
  const handleSaveEdit = async () => {
    try {
      const response = await fetch(`http://192.168.1.11:3003/project/updateP/${editedResponsibleId}`, { 
        method: 'PATCH',
        headers: {
